...
static GOptionEntry entries[] = {
    ...
    {"port", 'p', 0, G_OPTION_ARG_INT, &bindPort,
     "Specify port (default: 2425)", "PORT"},
...
int main(int argc, char** argv) {
  ...
  context = g_option_context_new(_("- A software for sharing in LAN (GTK4)"));
  ...
  if (version) {
    printf("iptux-plus: " VERSION "\n");
    exit(0);
  }

  string configPath = configFilename ? configFilename : getConfigPath4();
  ...
  Application4* app = new Application4(config);
  if (bindPort > 0 && bindPort < 65536) {
    app->getProgramData()->set_port(static_cast<uint16_t>(bindPort));
  }

  AdwApplication* adwApp = app->getApp();
  int ret = g_application_run(G_APPLICATION(adwApp), argc, argv);
  g_object_unref(adwApp);
  return ret;
}
